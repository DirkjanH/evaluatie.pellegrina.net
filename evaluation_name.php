...
if (empty($_GET['jaar']) OR $_GET['jaar'] == '') $_SESSION[ 'jaar' ] = date( 'Y' ); 
elseif ($_GET['jaar'] >= 2006 AND $_GET['jaar'] <= date( 'Y' ) ) $_SESSION[ 'jaar' ] = $_GET['jaar'];
...
if ( isset( $_POST[ 'cursusnr' ] )AND $_POST[ 'cursusnr' ] >= 0 AND $_POST[ 'cursusnr' ] <= 5 ) {
   $_SESSION[ 'cursusnr' ] = $_POST[ 'cursusnr' ] + $cursusoffset;
   if ( $_POST[ 'cursusnr' ] == 0 ) $_SESSION[ 'zoek_cursus' ] = '';
   else $_SESSION[ 'zoek_cursus' ] = "WHERE cursus = {$_SESSION['cursusnr']}";
}
...
$aantal_deelnemers = array( 0 => 88, 1 => 32, 2 => 56, 3 => 0, 4 => 0, 5 => 0 );
...
<div id="menu" class="w3-container w3-bar-block">
   ...
   <div id="navcontainer">
      <form action="" method="post" name="cursus_set" id="cursus_set">
         <input name="cursus" id="cursus" type="radio" <?php if (isset($_SESSION[ 'cursusnr']) and ($_SESSION[ 'cursusnr']=="0" )) echo 'checked'; elseif (empty($_SESSION[ 'cursusnr'])) echo 'checked'; ?> onClick="CursusZoek(0)">
   ...
   <div id="navcontainer">
      <form action="" method="post" name="vinden" id="vinden">
         ...
         <a href="javascript:Toon(<?php echo $naam['index']; ?>)" class="w3-bar-item w3-button w3-border-bottom w3-hover-blue w3-small">
            <?php 
					if ($naam['naam'] != NULL) echo $naam['naam']; else echo "???"; ?>
         </a>
         <?php	}?>
         <?php }?>
...
<div class="w3-container main">
   <table class="w3-table-all" id="opmerkingen">
      ...
      <?php if ($evaluatie['Lohmann'] != 0 or $evaluatie['Lohmann_tx'] != null) echo " 	<tr>
      <td>Antoinette Lohmann</td>
      <td colspan=\"2\">({$evaluatie['Lohmann']}) {$evaluatie['Lohmann_tx']}</td>
   </tr>"; ?>
      ...
      <?php if ($evaluatie['Semkiw'] != 0 or $evaluatie['Semkiw_tx'] != null) echo " 	<tr>
      <td>Marta Semkiw</td>
      <td colspan=\"2\">({$evaluatie['Semkiw']}) {$evaluatie['Semkiw_tx']}</td>
   </tr>"; ?>
      ...
      <?php if ($evaluatie['Vitale'] != 0 or $evaluatie['Vitale_tx'] != null) echo " 	<tr>
      <td>Marco Vitale</td>
      <td colspan=\"2\">({$evaluatie['Vitale']}) {$evaluatie['Vitale_tx']}</td>
   </tr>"; ?>
      ...
      <?php if ($evaluatie['Vacek'] != 0 or $evaluatie['Vacek_tx'] != null) echo " 	<tr>
      <td>Václav Vacek</td>
      <td colspan=\"2\">({$evaluatie['Vacek']}) {$evaluatie['Vacek_tx']}</td>
   </tr>"; ?>
